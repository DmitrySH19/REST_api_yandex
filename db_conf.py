...
class MyDatabase:
    ...
    def __init__(self, dbtype, username='', password='', dbname=''):
        ...
        if dbtype in self.DB_ENGINE.keys():
            ...
            complete_order_id = Table(
                'complete_order_id', self.meta, autoload=True)
    ...
    def insert_courier(self, courier, session):
        ...
        session.add(
            Couriers(id=courier['courier_id'], courier_type=courier['courier_type']))

        for j in range(len(courier['regions'])):
            session.add(
                Regions(courier_id=courier['courier_id'], region=courier['regions'][j]))
    ...
    def courier_patch_by_key(self, courier_id, field, session):
        ...
        if 'courier_type' in field.keys():
            edited = session.query(Couriers).filter_by(
                id=int(courier_id)).one()
            edited.courier_type = field['courier_type']
            session.add(edited)
        elif 'regions' in field.keys():
            delete = session.query(Regions).filter_by(
                courier_id=int(courier_id))
            for d in delete:
                session.delete(d)
            for j in range(len(field['regions'])):
                session.add(Regions(courier_id=int(courier_id),
                            region=field['regions'][j]))

        elif 'working_hours' in field.keys():
            delete = session.query(Working_hours).filter_by(
                courier_id=int(courier_id))
            for d in delete:
                session.delete(d)
    ...
    def get_couier_by_id(self, courier_id, session):
        ...
        if self.valid_id_courier(courier_id, session):
            ...
            courier_type = session.query(
                Couriers).filter_by(id=int(courier_id)).all()

            courier['courier_type'] = courier_type[0].courier_type
            regions = session.query(Regions).filter_by(
                courier_id=int(courier_id)).all()
            region_list = []
            for r in regions:
                region_list.append(r.region)
            courier['regions'] = region_list
            times = session.query(Working_hours).filter_by(
                courier_id=int(courier_id)).all()
            time = []
            for t in times:
    ...
    def get_order_by_id(self, order_id, session):
        if self.valid_order_id(order_id, session):
            ...
            order_weight = session.query(Orders).filter_by(
                order_id=int(order_id)).first()

            order['weight'] = float(order_weight.weight)
            order['region'] = order_weight.region

            times = session.query(Delivery_hours).filter_by(
                order_id=int(order_id)).all()
            time = []
            for t in times:
                time.append("{}-{}".format(t.start_hours, t.end_hours))
    ...
    def inset_complete_time(self, order_id, courier_id, complete_time, session):
        count_complite_orders = len(
            self.get_complete_orders(courier_id, session))
    ...
    def order_filtered(self, courier_id, session):
        ...
        cur_orders = [r.order_id for r in session.query(
            Complete_order_id.order_id).distinct()]
    ...
    def delete_uncomplete_by_id(self, order_id, session):
        ...
        delete = session.query(Complete_order_id).filter(
            Complete_order_id.order_id == order_id).one()
